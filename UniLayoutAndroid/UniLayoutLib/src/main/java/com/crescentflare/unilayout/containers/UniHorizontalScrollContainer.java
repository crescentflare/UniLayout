...
public class UniHorizontalScrollContainer extends HorizontalScrollView
{
    ...
    private void performLayout(Point measuredSize, int widthSize, int heightSize, int widthSpec, int heightSpec, boolean adjustLayout)
    {
        ...
        for (int i = 0; i < childCount; i++)
        {
            ...
            int viewWidthSpec = MeasureSpec.UNSPECIFIED;
            ...
            int y = getPaddingTop();
            if (isFillViewport() && widthSpec == MeasureSpec.EXACTLY)
            {
                resultWidth = Math.max(resultWidth, paddedWidthSize);
            }
            resultWidth = Math.max(viewMinWidth, resultWidth);
