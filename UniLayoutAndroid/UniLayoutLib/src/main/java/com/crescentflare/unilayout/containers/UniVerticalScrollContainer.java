...
public class UniVerticalScrollContainer extends ScrollView
{
    ...
    private void performLayout(Point measuredSize, int widthSize, int heightSize, int widthSpec, int heightSpec, boolean adjustLayout)
    {
        ...
        for (int i = 0; i < childCount; i++)
        {
            ...
            int viewHeightSpec = MeasureSpec.UNSPECIFIED;
            ...
            int y = getPaddingTop();
            if (isFillViewport() && heightSpec == MeasureSpec.EXACTLY)
            {
                resultHeight = Math.max(resultHeight, paddedHeightSize);
            }
            resultWidth = Math.min(viewWidthSize, Math.max(viewMinWidth, resultWidth));
            resultHeight = Math.max(viewMinHeight, resultHeight);
            if (viewLayoutParams instanceof MarginLayoutParams)
            {
